var HTML_ENTITY_CODEC = (function () {
    'use strict';

    var immuneCharacters = [',', '.', '-', '_', ' '],
        replacementHex = 'fffd',
        replacementChar = '\ufffd',
        characterToEntityMap,

        /**
         * Initializes the object that maps a decimal value
         * to an HTML entity name.
         */
        initCharacterToEntityMap = function () {
        ...
        /**
         * Returns the hexadecimal value of a non-alphanumeric character.
         *
         * @TODO: long description
         *
         * @param {String} char - The character to convert to a hex value.
         * @return {null|String} - If the character was alphanumeric,
         *     or the character code in hexadecimal, respectively.
         */
        getHexForNonAlphanumeric = function (char) {
        ...
        /**
         * Returns an encoded character using HTML entities.
         *
         * @TODO: long description
         *
         * @param {String} char - The character to be encoded.
         * @return {String} - The (encoded) character, depending whether
         *     or not encoding was necessary.
         */
        encodeCharacter = function (char) {
            var hex,
                entityName;

            // check for immune characters
            if (immuneCharacters.indexOf(char) !== -1) {
                return '' + char;
            }

            // check for alphanumeric characters
            hex = getHexForNonAlphanumeric(char);
            if (hex === null) {
                return '' + char;
            }
            ...
            if ((hex <= '1f' && char !== '\t' && char !== '\n' && char !== '\r')
                    || (hex >= '7f' && hex <= '9f')) {
                hex = replacementHex;  // Let's entity encode this instead of returning it
                char = replacementChar;
            }

            // check if there's a defined entity
            entityName = characterToEntityMap[parseInt(hex, 16)];
        ...
        /**
         * Returns an encoded string of text for (safe) HTML display.
         *
        ...
        encodeForHTML = function (input) {
            ...
            if (typeof characterToEntityMap === 'undefined') {
                initCharacterToEntityMap();
            }

            for (i = 0, len = input.length; i < len; i += 1) {
                char = input.charAt(i);
                encoded.push(encodeCharacter(char));
            }
